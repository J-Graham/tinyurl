...
var MONGODB_URI = 'mongodb://'+process.env.USER+':'+process.env.PASS+'@'+process.env.HOST+':'+process.env.MONGOPORT+'/'+process.env.DB;
var collection;
...
function getId(id){
 return new Promise(function (resolve, reject) {
    ...
    collection.findOne({"_id":parseInt(id)}, function (err, data) {
          if (err) {
            reject(new DatastoreUnderlyingException(id, err));
          } else {
            try {
              if(data===null){
                resolve(null);
  ...
  return new Promise(function (resolve, reject) {
    if (typeof(key) !== "string") {
      ...
      try {
        ...
        collection.insert(value, function (err, res) {
          if (err) {
...
function getIdCallback(id, value, callback){
    getId(id, value)
    .then(function (value) {
      callback(null, value);
    })
...
function getIdSync(key, value) {
  return sync.await(getIdCallback(key, value, sync.defer()));
}
